...
- *What it produced:* Complete `generate.py` with strict grounding system prompt, primary/fallback model logic (`llama-3.3-70b-versatile` → `llama-3.1-8b-instant`), and temperature `0.0` for deterministic output; complete `app.py` with custom CSS survival-horror theme (crimson red, Share Tech Mono font, dark background), `@st.cache_resource` backend initialization, game filter dropdown, answer card, deduplicated grouped sources display, and a developer debug expander.
